...
Shader "UC/Actor/Protagonist/High/ToonSkin"
{
    Properties
    {
...
/*              [Space]
                ...
                _OutlineZOffset("内描边拉没",Range(0.0,0.01)) = 0.0
                // _OutlineMax("描边矫正限制",Range(0.0,1.0)) = 0.5
                [HideInInspector] _OutlineBaseThickness ("描边宽度", Range(0,0.08)) = 0.05 //0.0015
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half3 ComputeTextureSpaceNormal(float2 uv,half3 vertex_nor,half3 vertex_tangent)
            {
                ...
                float4 n = var_NMTex;
                float3 normalTS =  UnpackNormalScale(n,0.0);// 法线图
            ...
            float4 frag(Varings IN):SV_Target
            {
                ...
                float DetailBRDF = RemapFloatValue(0, 1, _SkinDetailShadowMin, _SkinDetailShadowMax, saturate(NoL)) / _SkinDetailShadowMax * ToonSelfShadow;
                half3 diffuse = DetailBRDF;
                diffuse.rgb *= pow(baseMap, _BaseColorPower) * _BaseColor.rgb * light.color;
                
                half3 specular = ToonBRDF.r * _SpecularDensity;

                diffuse = DetailBRDF * SAMPLE_TEXTURE2D(_ToonBRDFTexture, sampler_ToonBRDFTexture, float2(saturate(NoL * 0.5 + 0.5), 0)).g * pow(baseMap, _BaseColorPower) * _BaseColor.rgb * light.color;
                diffuse = RemapFloat3Value(0, 1, 0, 3, diffuse);
                ...
                half3 bakeGI = SampleSH(N);
                
                OutColor.rgb = (diffuse + specular) * bakeGI * 2.0f;
                
                clip(baseMap.a-_Cutoff);
                //return OutColor * ShadowMask;
                return OutColor;
            }
            ENDHLSL            
        }
