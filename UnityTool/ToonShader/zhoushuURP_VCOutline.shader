  ...
  //      SAMPLER(sampler_BumpMap);
        ...
        "LightMode" = "UniversalForward"
          ...
          //      half4 LightMapColor = SAMPLE_TEXTURE2D(_LightMap, sampler_LightMap, TRANSFORM_TEX(i.uv0 ,_LightMap) );
                ...
                float mapCap = SAMPLE_TEXTURE2D(_MetalMap , sampler_MetalMap , i.matcapUV).r;
                float3 mapCapColor =saturate(  smoothstep(   _SpecMulti - mapCap ,_SpecMulti + mapCap , mapCap) * _MetalIntensity *_SpecColor);
             ...
             //   Ramp *= regularShadow;
                ...
                // half3 finalRamp = lerp(RampSun , RampNight , _RampChange) * baseMap.rgb;
...
//===========================================================================================================
         ...
         half3 brdfSpecular = lerp(kDieletricSpec.rgb, diff, _Metalic);
                
    
            half3 bakeGI = SampleSH(nDir);
            half3  diffseColor = diff * oneMinusReflectivity;
            half3 indirectDiffuse = bakeGI * diffseColor;
...
//===========================================================================================================
    ...
    // Specular-----------------------------------------------------------------------------------  
                 ...
                 half3 specularColor = _EnableSpecular   * mapCapColor ; //* brdfSpecular;
         

      //-------------------------------------------------------------------------------------   
        
               half shadow =lerp( (1-_ShadowStrength) ,1, mainLight.shadowAttenuation * mainLight.distanceAttenuation );
