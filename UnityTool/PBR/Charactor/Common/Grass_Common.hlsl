...
float2 GrassAnim(float3 positionWS , float3 positionOS)
{
    ...
    float2 offset=  sin(_Time.y *_WindFrequency  - rand)  * _WindSpeed *  (max(0,positionOS.y) *_WindStrength) ;
    return offset;
...
float2 WindAnim(float3 positionWS ,float3 positionOS )
{
    ...
    float2 offset = RemapFloatValue(-1,1,0,1 ,sin(_Time.y* _WindFrequency + rand ) * _WindSpeed  )* Y *half2(-0.1,-0.35);
    return offset;
}
...
float3 GrassColorGrid(float3 albedo , float pos_Y , float ao  , float3 W)
{
    float3 color = 0;
    #ifdef _GRASS
        color  =lerp(_ColorGridBottom,  _ColorGridTop, smoothstep(_ColorGridOffset, _ColorGridOffset + _ColorGridSmooth, pos_Y));
    ...
    )
{
    ...
    float3 col = Desaturation(brdfData.diffuse , _SubsurfaceSaturation);
    ...
    half3 brdf = 0;
        ...
        brdf += brdfData.diffuse * lightColor * _DirectBright * lightAttenuation;
    #endif
    ///========================================================================================================
