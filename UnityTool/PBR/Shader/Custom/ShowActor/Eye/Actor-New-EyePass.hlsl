#ifndef CUSTOM_NEW_PASS_INCLUDED_EYE
    ...
    half4 CustomActerPBR(InputData inputData, SurfaceData surfaceData)
    {
        ...
        mainLight, inputData.normalWS, inputData.viewDirectionWS
            ...
            return half4(directColor + indirectColor + surfaceData.emission * max(0.4,(sin(_Time.z * _BreatheSpeed) + 1.25)), surfaceData.alpha);
        #else
    ...
    Varyings LitPassVertex(Attributes input)
    {
        ...
        // #if defined(_EMISSION_DYNAMIC)
            // output.uv.zw = TRANSFORM_TEX(input.lightmapUV, _WaveMap);
            // output.uv.zw += float2(0.0,-_Time.x * _WaveSpeed);
        // #endif
    ...
    //   half3 msa = SAMPLE_TEXTURE2D(_MSAMap, sampler_MSAMap,input.uv.xy).rgb;
        ...
        InitializeStandardLitSurfaceData(input.uv, surfaceData ,eye);
