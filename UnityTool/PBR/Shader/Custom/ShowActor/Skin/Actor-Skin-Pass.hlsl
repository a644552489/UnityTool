#ifndef CUSTOM_SKIN_PASS_INCLUDED
    #define CUSTOM_SKIN_PASS_INCLUDED
    ...
    half3 normalWS, half3 viewDirectionWS)
    {
        ...
        half3 Specular = brdfData.specular * DirectBRDFSpecular(brdfData, normalWS, lightDirectionWS, viewDirectionWS) * lightAttenuation * saturate(NoL);

        float ShadowMask = lightAttenuation * WrapNoL;
        half SSSUV = float2(ShadowMask, 0.2);
        half3 SSS = SAMPLE_TEXTURE2D(_3SRampMap, sampler_3SRampMap, SSSUV);
        //SSS = RemapFloat3Value(float3(0,0,0), float3(1,1,1), float3(0.2,0.2,0.2), float3(1,1,1), SSS);
        
        return (Diffuse * SSS + Specular) * lightColor;
    }
    ...
    half4 CustomActorPBR(InputData inputData, SurfaceData surfaceData)
    {
        BRDFData brdfData ;
        ...
        half3 IndirectLighting = CustomGI(brdfData, inputData.bakedGI, surfaceData.occlusion, inputData.normalWS, inputData.viewDirectionWS);
        
        half3 DirectLighting = CustomLightingPhysicallyBased(brdfData, mainLight, inputData.normalWS, inputData.viewDirectionWS);
            ...
            for (uint lightIndex = 0u; lightIndex < pixelLightCount; ++lightIndex)
            {
                ...
                #endif
                DirectLighting += CustomLightingPhysicallyBased(brdfData, 
        ...
        return half4(DirectLighting + IndirectLighting + surfaceData.emission, surfaceData.alpha);  
    ...
    half4 LitPassFragment(Varyings input) : SV_Target
    {
        ...
        half4 color = CustomActorPBR(inputData, surfaceData);
